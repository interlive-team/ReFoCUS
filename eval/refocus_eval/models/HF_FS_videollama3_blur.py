...
@register_model("VIDEOLLAMA3_FRAME_SELECTION_BLUR")
class VIDEOLLAMA3_FRAME_SELECTION_BLUR(lmms):
    def __init__(
        self,
        pretrained: str,
    ...
    def _apply_gaussian_blur_inplace(
        self, frames_np: np.ndarray, blur_flags: np.ndarray
    ):
        """
        frames_np: (T, H, W, C) uint8
        blur_flags: (T,) bool
        """
        if self.blur_sigma > 0.0 and np.any(blur_flags):
